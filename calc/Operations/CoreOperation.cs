...
namespace calc
{
    internal class CoreOperation : Operation
    {
        ...
        private float g = 7; // g represents the precision desired, p is the values of p[i] to plug into Lanczos' formula
        ...
        { 
            if (n < 0.5)
            {
                return (float) (Math.PI / Math.Sin(n * Math.PI) / FactorialOf(1 - n));
            }
            else
            {
                n--;
